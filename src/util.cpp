...
#else
    const char* pszModule = "CharlieBrownCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CharlieBrownCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CharlieBrownCoin
    // Mac: ~/Library/Application Support/CharlieBrownCoin
    // Unix: ~/.CharlieBrownCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CharlieBrownCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "CharlieBrownCoin";
#else
    // Unix
    return pathRet / ".CharlieBrownCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CharlieBrownCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CharlieBrownCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong CharlieBrownCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("CharlieBrownCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
